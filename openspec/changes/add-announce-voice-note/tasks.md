...
- [x] 7.5 Added during verification: exercise the audio path through the live daemon, which 7.1 could not reach. Speech output reported available (af_heart, CPU, default device at 24 kHz). Three cases run one at a time against the real socket, confirmed by ear and by the daemon's own `heard_all`: a voice note spoke as written, a message with no element spoke the extract, an empty element opened no session and made no sound. Repeated through the installed script under `/usr/bin/python3`, the interpreter the registration actually names.
