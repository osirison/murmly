...
- [x] 10.1 Add a Qt renderer speaking the existing newline-delimited JSON protocol over the socketpair, so the daemon does not learn which renderer it started
- [x] 10.2 Add PySide6 to `pyproject.toml` behind the platform markers for the platforms whose renderer needs it
- [x] 10.3 Set `Qt.WindowTransparentForInput` and `Qt.WindowDoesNotAcceptFocus`, and apply `WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW` to the native `HWND` through `SetWindowLongPtr`
...
- [x] 11.4 If it does carry the defect, resolve a Windows espeak-ng install and name it in the remedy through the existing "runtime absent" reporting path; if it does not, use the bundled library
...
- [x] 16.2 Reduce the per-platform entry points to a bootstrap — install `uv`, then hand off — as `sh` and as PowerShell (`bootstrap.sh` done and tested on Linux; `bootstrap.ps1` is now parsed and its argument handling exercised against a stubbed `uv` in `tests/test_bootstrap_ps1.py`, which also caught and fixed a real defect: `Write-Error` on the "still not on PATH" branch was a terminating error under the script's own `$ErrorActionPreference = "Stop"`, so its `return 1` never ran)
...
- [x] 19.1 Edit the `recording-overlay` capability's `## Purpose` line in `openspec/specs/recording-overlay/spec.md` directly — a delta's Purpose is ignored for an existing capability, so it does not change at archive time
- [x] 19.2 Rewrite `manual/what-you-need.md` to describe three platforms rather than one distribution, keeping the KDE and Wayland detail as one platform's entry rather than the whole story
- [x] 19.3 **Revised by the binding macOS scope decision, which postdates this task's own text.** That text named Intel macOS as a special case of an unsupported machine, which implied Apple Silicon macOS was supported — it is not; macOS is out of scope entirely, full stop, not documented as a platform at all. What `manual/what-you-need.md` now states instead: Murmly runs on Linux and Windows; macOS is not supported; and within Linux and Windows, musl Linux and Windows on ARM64 each have no build of `ctranslate2`, the transcription runtime, and murmly refuses to sync on either, naming why
- [x] 19.4 Document each platform's permissions in `manual/what-you-need.md` and `manual/install.md`: the Windows microphone privacy settings (the two `CapabilityAccessManager` toggles under Settings → Privacy & security → Microphone) and the Linux KDE Wayland paste-permission dialog. The macOS Accessibility and microphone grants are not documented — macOS is out of scope per the binding decision
- [x] 19.5 Document in `manual/where-your-words-go.md` that pasting into an elevated window on Windows silently does nothing, in the same place the KDE input-consent dialog is documented, since it is the same class of failure
- [x] 19.6 Give `manual/install.md` the per-platform install path, and `manual/changing-your-hotkey.md` the per-platform hotkey story, including the desktops where the binding is not automatic
- [x] 19.7 Carry the same disclosures onto every page that shows an install command, since the requirement binds each such page and not only the landing page — `manual/install.md`, `manual/making-murmly-speak.md`, and `manual/speed-and-memory.md` in addition to `README.md` and `site/index.html`
- [x] 19.8 Update `pyproject.toml`'s description, `README.md`'s opening and its requirements paragraph, and the landing page in `site/`: Murmly is no longer Fedora-first. Also fixed two places carrying the same stale claim that the task did not name: `src/murmly/cli.py`'s `argparse` description (`murmly --help`'s own banner) and `mkdocs.yml`'s `site_description`/`copyright`, which render into every manual page's `<meta>` tag and footer
- [x] 19.9 **Revised by the same binding scope decision.** The task text's list included a silent macOS microphone, an ungranted Accessibility paste, and a hotkey the frontmost macOS application consumes — all macOS, all out of scope, none added. `manual/troubleshooting.md` gets only the Windows failures: a paste discarded by UIPI into an elevated window, and a hotkey collision Windows cannot name the owner of (unlike KDE); plus a GNOME-hotkey-unverified note, since GNOME is a real, in-scope Linux desktop this change adds
- [x] 19.10 Field note recorded: `docs/agent-notes/hotkey-record-written-on-every-platform.md` — `manual/install.md`'s "installing murmly writes exactly three files" was stale before this pass touched it. Task 5.4's hotkey record (`~/.config/murmly/hotkeys.json` on Linux, `%APPDATA%\murmly\hotkeys.json` on Windows) is written unconditionally on every platform, including KDE and GNOME where the desktop also holds its own copy of the binding — a fourth file the install docs never mentioned
