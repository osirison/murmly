...
### Requirement: Previous clipboard restored only after the transcript is consumed

When clipboard restoration is enabled and delivery was injected, Murmly SHALL restore the previous clipboard contents only after the receiving application has taken the transcript from the clipboard, or after a bounded wait has elapsed. Murmly MUST NOT restore the previous contents while the receiving application may still be reading the transcript.

#### Scenario: Receiving application reads the transcript promptly

- **WHEN** the receiving application takes the transcript from the clipboard
- **THEN** Murmly restores the previous clipboard contents

#### Scenario: Receiving application never reads the transcript

- **WHEN** the receiving application has not taken the transcript once the bounded wait elapses
- **THEN** Murmly restores the previous clipboard contents
- **AND** Murmly returns to idle without waiting further
...
On a session where the focused window cannot be observed, Murmly SHALL inject the paste as it does today rather than refusing every delivery. Such a session MUST still receive clipboard preservation and consumption-ordered restoration, and MUST NOT report itself as verified.
...
- **AND** the previous clipboard is restored only after consumption or the bounded wait
...
Murmly SHALL provide a configuration option that disables delivery target verification. When verification is disabled, Murmly MUST inject the paste without comparing the focused window, and MUST retain clipboard preservation and consumption-ordered restoration.
