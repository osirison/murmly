...
- [ ] 1.1 Add `unload_after_idle_s` to the `[stt]` section in `src/murmly/config.py`, bounded 30-86400, **defaulting to `300`** (five minutes, enabled), alongside `lazy_load_model`. `0` disables
- [ ] 1.2 Add `unload_after_idle_s` to the `[tts]` section, bounded 30-86400, **defaulting to `0`** (disabled)
...
- [ ] 1.5 Tests: `[stt]` defaults to 300 and `[tts]` to 0 when absent; an explicit `0` disables either; an out-of-range value falls back to that setting's own default, not to a shared one
...
- [ ] 4.4 Make a resolved value of `0` register no timer at all, so synthesis is inert by default and either model can be switched off entirely
...
- [ ] 6.4 Confirm a default install with neither setting configured releases the transcription model after five idle minutes and holds the synthesis session indefinitely, and that `[stt] unload_after_idle_s = 0` restores today's always-resident behaviour
- [ ] 6.5 Measure at least eight transcription unload/reload cycles and confirm no per-cycle drift, against the baseline taken before this default was chosen: GPU returning to exactly 2240 MiB and releasing to 160 MiB each cycle, host RSS drift flat at +2.3 MiB from cycle 1 onward, unload 55-57 ms, reload 740-771 ms. Per-cycle growth here would reach every install, unlike the synthesis path, so this gates the default rather than merely documenting it
- [ ] 6.6 Measure host RSS across at least five synthesis release/rebuild cycles under both `[tts] device` values and check it against the drift table in `design.md` — Risks. Expect the `cpu` path to oscillate and return to baseline, and the `cuda` path to take a one-time step of roughly 277 MiB and then creep about 8 MiB per cycle. If the `cuda` creep is materially steeper than measured, the synthesis timer needs a guard rather than only documentation
