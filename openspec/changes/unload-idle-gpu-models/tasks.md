...
- [ ] 2.3 Add a release method that acquires `_model_lock`, calls `unload_model(to_cpu=True)`, and is a no-op when the model is absent or already unloaded
...
- [ ] 6.1 Run the full suite: `uv run --extra cuda python -m unittest discover -s tests`
