...
- [ ] 8.3 Restart the service while a transcription is in flight and confirm the hotkey path reports a message rather than a traceback — run `uv run --extra cuda python scripts/verify_restart_mid_transcription.py`, which waits for the daemon to report THINKING and restarts inside that window rather than hand-timing the restart, and reports the caller's outcome as answered, no response, traceback, or raced. Not yet run: it records from the microphone and needs a live session. The transport half is covered automatically by `test_shutdown_answers_a_command_that_outlives_the_drain`, `test_shutdown_waits_for_a_write_that_outlives_the_drain`, and `test_a_client_that_receives_nothing_raises_a_named_type`, each over a real socket; what the script adds is that a `systemctl --user restart` produces the same interleaving as an in-process `shutdown()`
