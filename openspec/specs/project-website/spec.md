...
Defines the public site that introduces Murmly to someone who has never seen it and then documents it for someone who has installed it. It covers the landing page — what it must show and in what order — and the documentation pages that hold every piece of reference material a person needs after a first install, which is the only place that material exists. It covers what any page is allowed to claim, what it must disclose about the machine Murmly actually needs, where and how the site is published, what stands between its committed sources and its published bytes, and the constraints that keep every page self-contained, readable without JavaScript, and free of any request to a third party. Those constraints bind whatever a generator or a theme contributes as firmly as they bind hand-written markup.
